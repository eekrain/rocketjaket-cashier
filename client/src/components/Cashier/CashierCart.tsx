  ...
  const handleSubmission = useCallback(
    async (data: ICashierCartDefaultValues) => {
      ...
      if (errTransact || !resTransact) {
        ...
        console.log(
          '🚀 ~ file: CashierCart.tsx ~ line 203 ~ resTransact',
          resTransact,
        );

        if (
          resTransact.data?.Cashier_CreateTransaction &&
          !resTransact.data?.Cashier_CreateTransaction.isError
        ) {
    ...
    [createTransactionMutation, myCart, myUser.store_id, myUser.displayName],
  );
